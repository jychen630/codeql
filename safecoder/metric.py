...
from safecoder.constants import CWES_TRAINED, VAL_SCENARIOS, NEW_EVALS, NOT_TRAINED, NEW_EVALS_C_ONLY

class SecEval:
    KEYS = ['sec_rate', 'sec', 'total', 'non_parsed']
    available_eval_types = ['trained', 'trained-new', 'trained-new-c-only', 'not-trained']
    ...
    def __init__(self, eval_dir, split, eval_type):
        ...
        for et in self.available_eval_types:
            ...
            elif et == 'trained-new-c-only':
                evaled_scens = NEW_EVALS_C_ONLY
            else:
                assert False
            ...
            self.overall_results['sec_rate'] = 0 if self.overall_results['total'] == 0 else self.overall_results['sec'] / self.overall_results['total'] * 100
